...
```{r fig.align="center", fig.cap="Total expenditure by coarse description", tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
by=list(office=Debt2$office, legislativedistrict=Debt2$legislativedistrict,
position=Debt2$position, party=Debt2$party, jurisdiction=Debt2$jurisdiction, jurisdictiontype=Debt2$jurisdictiontype), FUN=sum)
...
countbyoffice <- aggregate(sumbycand['position'], by=list(office=sumbycand$office),
FUN=length)
...
Count_amount = length(amount), Mean_amount = mean(amount),
...
```
```{r fig.align="center", fig.cap="Fraction of party expenditure by coarse description, by party"}

mycolors=c("gray25","gray45","gray75")
par(mar=c(12,6,4,2)+0.1)
barplot(jxtab_sum_mat,
        beside = TRUE,
        main = "Fractional Expenditures by Coarse Description, by Party",
        xlab = "",
        ylab = "", cex.names=0.6, las=2, col=mycolors)
legend("topright", rownames(jxtab_sum_mat), cex=0.75, pch=0, pt.cex = 0, fill=mycolors)
