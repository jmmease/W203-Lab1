...
```{r}
t(DebtShifted[1,c('filerid', 'filername', 'office', 'DUMMY', 'legislativedistrict', 'party', 'jurisdiction', 'jurisdictiontype')])
```
...
To move forward, we will define two new data sets
 ...
 2. `DebtRight` will include the unique, non-missing, rows for the columns from `DUMMY` through `vendorstate`. Additionally, we will rename `DUMMY` to `office` in `DebtRight`, since it has a nearly identical set of factor levels as `office` in `DebtLeft`, and seems to serve the same purpose.
